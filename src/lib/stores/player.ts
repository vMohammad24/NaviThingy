...
function startMpvStatusPolling() {
	...
	const updatePoll = async () => {
		const now = Date.now();
		if (
			mpvStatusThrottle.isPolling ||
			now - mpvStatusThrottle.lastPoll < mpvStatusThrottle.minInterval
		) {
			return;
		}
		...
		try {
			...
			if (this.useMpv && (this.mpvInitialized || (await initMpv())) && invoke) {
				if (this.useNativeMpvPlaylist) {
					...
					const status = await invoke<{ playlist_count: number; playlist_pos: number }>(
						'mpv_get_status'
					);

					if (status.playlist_count !== streamUrls.length) {
						console.warn(
							`Playlist length mismatch: expected ${streamUrls.length}, got ${status.playlist_count}`
						);
					}
...
function createPlayerStore() {
	...
	cl.subscribe(async (newClient) => {
		...
		if (newClient) {
			...
			const startIndex = queue.current ? queueEntry.findIndex((t) => t.id === queue.current) : 0;
	...
	audioPlayer.onEnded(() => {
		update((state) => {
			...
			if (
				!(
					audioPlayer.isMpvEnabled() &&
					audioPlayer.hasMpvPlaylistLoaded() &&
	...
	mpvStatus.subscribe((status) => {
		if (audioPlayer.isMpvEnabled()) {
			update((state) => {
				...
				if (
					(status.state === 'paused' && state.isPlaying) ||
				...
				if (status.state === 'ended' || status.state === 'idle') {
					if (state.currentIndex === state.playlist.length - 1 && state.repeat !== 'all') {
						return {
							...state,
							isPlaying: false
						};
	...
	return {
		...
		next: () => {
			update((state) => {
				...
				if (
					audioPlayer.isMpvEnabled() &&
					get(mpvSettings).nativePlaylist &&
					audioPlayer.hasMpvPlaylistLoaded()
				) {
					if (invoke) {
						try {
							...
							if (nextIndex === 0 && state.repeat === 'all') {
								invoke('mpv_playlist_jump_to_index', { index: 0 }).catch((error) =>
									console.error('Failed to jump to first track:', error)
								);
							} else {
								invoke('mpv_playlist_jump_to_index', { index: nextIndex }).catch((error) =>
									console.error('Failed to navigate to next track:', error)
								);
							}
				...
				if (
					...
					if (invoke) {
						try {
							...
							invoke('mpv_playlist_jump_to_index', { index: prevIndex }).catch((error) =>
								console.error('Failed to navigate to previous track:', error)
							);
