...
function createThemeStore() {
	const stored =
		typeof localStorage !== 'undefined' ? localStorage.getItem(key) || 'default' : 'default';

	const { subscribe, set } = writable<string>(stored);

	const storedCustomThemes =
		typeof localStorage !== 'undefined' ? JSON.parse(localStorage.getItem(customKey) || '[]') : [];
