<script lang="ts">
	...
	let lyrics = $state<{ synced: boolean; plain: string; lines: SyncedLyric[] } | undefined>();
	...
	function scrollToCurrentLyric() {
		...
		setTimeout(() => {
			try {
				...
				if (currentLyricIndex >= 0 && elements && elements.length > currentLyricIndex) {
					currentLyricElement = elements[currentLyricIndex] as HTMLParagraphElement;
...
{#if $player.currentTrack}
	...
	<div
		class={`fixed left-0 right-0 z-50 ${
			isFullscreen ? 'bottom-full opacity-0 pointer-events-none' : 'bottom-0 opacity-100'
		} transition-all duration-300 ease-out`}
		style={isDraggingPanel && !isFullscreen ? `transform: translateY(${currentTranslateY}px)` : ''}
	>
		<button
		...
		<div class="bg-surface/95 backdrop-blur-md border-t border-white/5 shadow-up relative z-10">
			{#if duration > 0}
			...
			<div class="flex items-center p-3.5 px-4">
				<!-- svelte-ignore a11y_click_events_have_key_events -->
				<!-- svelte-ignore a11y_no_static_element_interactions -->
				<div class="flex items-center gap-4 flex-1 min-w-0" onclick={openFullscreen}>
	...
	{#if isFullscreen}
		<div class="fixed inset-0 z-50 flex flex-col" transition:fade={{ duration: 300 }}>
			<div class="pt-safe sticky top-0 z-10">
				<div class="flex items-center justify-between p-4">
					...
					<div class="text-center">
						<h2 class="text-sm font-medium uppercase tracking-wider text-white/80">Now Playing</h2>
					</div>
				...
				{:else}
					<div class="flex flex-col h-full p-4 pt-0">
						<div class="flex-1 flex items-center justify-center">
							<div class="w-full max-w-xs" in:fade={{ duration: 200, delay: 100 }}>
						...
						<div class="mt-6 mb-4" in:fade={{ duration: 200, delay: 200 }}>
							<div class="flex items-center justify-between">
								<div class="flex-1">
									<h1 class="text-xl font-bold text-white leading-tight truncate">
										{$player.currentTrack.title}
									</h1>
										...
										<a
											...
											onclick={() => {
												...
												if ($player.currentTrack) {
													...
													setTimeout(() => (window.location.href = `/artists/${artistId}`), 100);
												}
											}}
										>
											{$player.currentTrack.artist}
											...
											<a
												...
												onclick={() => {
													...
													if ($player.currentTrack) {
														...
														setTimeout(() => (window.location.href = `/albums/${albumId}`), 100);
													}
												}}
											>
							...
							<div class="flex items-center justify-between mb-4">
								<button
									class={`p-2 rounded-full transition ${
										$player.shuffle ? 'text-accent' : 'text-white/60 hover:text-white/80'
									}`}
								...
								<button
									class={`p-2 rounded-full transition ${
										$player.repeat !== 'none' ? 'text-accent' : 'text-white/60 hover:text-white/80'
									}`}
			...
			{#if !showQueueInFullscreen && lyrics && showLyrics}
				...
				>
					...
					<div class="px-6 py-2 flex items-center justify-between border-b border-white/10">
						<h3 class="text-base font-medium text-white/90 uppercase tracking-wider">Lyrics</h3>
						<button
					...
					>
						{#if lyrics}
							...
							{:else if lyrics.plain}
								<div class="px-2">
									<p class="text-base text-white/80 whitespace-pre-wrap leading-relaxed">
										{lyrics.plain}
									</p>
								</div>
							{:else}
								<div class="flex flex-col items-center justify-center h-full">
									<div class="bg-white/5 rounded-xl p-8 text-center">
										...
										<p class="text-xs text-white/40">
											Try searching online for lyrics to "{$player.currentTrack.title}"
										</p>
									</div>
								</div>
							{/if}
