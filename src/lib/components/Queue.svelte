<script lang="ts">
	...
	function handleDndFinalize(e: CustomEvent<{ items: any[] }>) {
		...
		const newPlaylist = [...$player.playlist.slice(0, $player.currentIndex + 1), ...cleanItems];
...
>
	{#if !minimal}
		...
		>
			...
			<div class="flex flex-col items-center md:items-start md:flex-row md:gap-3">
				<div
	...
	<div class="flex-1 overflow-y-auto transition-all scroll-smooth scrollbar-none md:pb-20">
			...
			{#if $history.length > 0}
				...
				<div class="opacity-60">
					{#each $history as track, i}
						...
						>
							<div class="relative w-12 h-12 rounded-lg overflow-hidden shadow-md md:w-14 md:h-14">
								<img
									src={track.coverArt}
									alt={track.title}
							...
							{#if $player.isPlaying}
								<div class="absolute inset-0 flex items-center justify-center bg-black/30">
									<div class="w-8 h-8 flex items-center justify-center">
										<div class="w-1 h-5 bg-white/80 mx-0.5 animate-eq-bar1"></div>
										<div class="w-1 h-5 bg-white/80 mx-0.5 animate-eq-bar2"></div>
										<div class="w-1 h-5 bg-white/80 mx-0.5 animate-eq-bar3"></div>
									</div>
								</div>
							{/if}
						</div>
						...
						>
							...
							<div class="relative w-12 h-12 rounded-lg overflow-hidden shadow-md md:w-14 md:h-14">
								...
								<div
									class="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
								>
									<Play size={20} class="text-white md:w-6 md:h-6 group-hover:animate-scale-in" />
								</div>
							</div>
