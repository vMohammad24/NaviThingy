<script lang="ts">
	...
	import { Disc, Home, ListMusic, Menu, Music, Settings, Tag, Users, X } from '@lucide/svelte';
...
>
	...
	>
		<div class="flex flex-col p-3 relative z-10 h-full">
			<div class="mb-6 flex items-center justify-between {isOpen ? 'px-2' : 'justify-center'}">
				{#if isOpen}
            ...
            hover:shadow-[0_0_20px_-3px_rgba(var(--color-primary-rgb),0.5)] active:scale-95"
				...
				>
					...
					{:else}
						<Menu
							size={24}
							class="transition-transform duration-500 ease-out {isOpen ? 'rotate-180' : ''}"
						/>
					{/if}
				</button>
			</div>
			...
			<nav class="flex flex-col gap-2 {isOpen ? 'px-2' : ''} flex-grow">
              ...
              active:scale-95
              {page.url.pathname === item.href ? 'bg-white/[0.03] shadow-lg' : ''}"
					>
