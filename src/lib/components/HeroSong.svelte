<script lang="ts">
	...
	import { CalendarRange, Clock1, Gauge, Music, Pause, Play, SkipForward } from '@lucide/svelte';
	...
	let isPlaying = $derived($player.currentTrack?.id === song?.id && $player.isPlaying);
	...
	>
		...
		<div class="flex flex-col md:flex-row gap-6 sm:gap-10 relative">
			...
			<div class="md:w-2/3 space-y-6 sm:space-y-8">
				<div class="h-8 sm:h-10 bg-surface/30 rounded-xl w-3/4 animate-pulse"></div>
				<div class="h-6 sm:h-8 bg-surface/30 rounded-xl w-1/2 animate-pulse"></div>
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
					{#each Array(4) as _}
						<div class="h-12 sm:h-14 bg-surface/30 rounded-xl animate-pulse"></div>
					{/each}
				</div>
			</div>
		</div>
		...
		<div class="relative z-10">
			<div class="flex flex-col md:flex-row gap-8 md:gap-10 lg:gap-14">
				<div class="md:w-1/3 flex flex-col items-center gap-6">
					<div class="relative group w-full max-w-[320px] min-w-[280px] mx-auto">
						<div
