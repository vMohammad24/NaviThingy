<script lang="ts">
	...
	let isPlaying = $derived($player.currentTrack?.id === album.id && $player.isPlaying);
	...
	>
		...
		{#if showMetadata}
			<div class="p-3 sm:p-4 md:p-6 space-y-1 sm:space-y-2">
				...
				{#if isPlaying}
					<div class="h-1 w-full bg-surface/30 rounded-full overflow-hidden mt-2 sm:mt-4">
						<div class="h-full bg-primary rounded-full w-1/2 animate-progress"></div>
					</div>
				{/if}
			</div>
		{/if}
