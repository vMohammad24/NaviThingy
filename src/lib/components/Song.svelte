<script lang="ts">
	...
	function handleMenuAction(action: string) {
		switch (action) {
			...
			case 'download':
				(async () =>
					download(await $client!.download(song.id), `${song.artist} - ${song.title}`))();
				break;
			case 'favorite':
				if (song.starred) {
	...
	<div class="relative z-10 flex items-center gap-2 sm:gap-4 w-full">
		<div class="w-8 sm:w-12 flex items-center justify-start">
            ...
            flex items-center justify-center group/btn hover:scale-110 active:scale-95"
				...
				>
					{#if isCurrentlyPlaying(song)}
						<Pause size={16} class="text-primary group-hover/btn:scale-90 transition-transform" />
					{:else}
						<Play
							size={16}
			...
			{:else if showTrackNumber}
				<span class="w-6 sm:w-8 text-center text-xs sm:text-sm text-text-secondary/60 font-medium">
					{index + 1}
				</span>
			{/if}
		</div>
		...
		<div class="flex-grow flex flex-col min-w-0 max-w-[50%] sm:max-w-none">
			<div class="flex items-center gap-2">
				<span class="font-medium truncate text-sm sm:text-base">{song.title}</span>
				{#if song.starred}
