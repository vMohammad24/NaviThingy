<script lang="ts">
	...
	onMount(async () => {
		try {
			songData = await $client!.getSong(id, true);
			loading = false;
		} catch (e) {
			...
			<div class="md:w-1/3">
				...
				<div class="mt-4">
					...
					<a
						...
						href={`/artists/${songData.song.artistId}`}>{songData.song.artist}</a
					>
			...
			<div class="md:w-2/3 bg-surface rounded-lg p-4 mt-6 md:mt-0">
				...
				<div class="divide-y divide-primary/20">
					{#each (songData.similarSongs || []).sort((a, b) => (a!.track ?? 0) - (b.track ?? 0)) as song, i}
						<Song {song} index={i} playlist={songData.similarSongs || []} showTrackNumber={false} />
					{/each}
				</div>
			</div>
		</div>
