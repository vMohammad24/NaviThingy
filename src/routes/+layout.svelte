<script lang="ts">
	...
	import { isMobile, sidebarHidden, sidebarOpen } from '$lib/stores/sidebarOpen';
	...
	let currentTheme = $derived(theme.themes.find((t) => t.id === $theme) || theme.themes[0]);
...
{#if !$isMobile}
	<div
		class="fixed top-0 right-0 z-50 h-8 flex w-screen justify-end {$sidebarHidden ? 'hidden' : ''}"
